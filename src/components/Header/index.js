    ...
    <header className="z-10 fixed w-full flex justify-between items-center bg-darkPurple text-white px-6 py-3">
      <div className="flex items-center gap-1">
        {formatCounter(counter).map((digit, index) => (
          <div key={index} className="font-extrabold text-2xl text-darkPurple bg-white rounded w-6 flex items-center justify-center">
            {index === 2 ? ':' : digit}
          </div>
        ))}
      </div>
      ...
      <div className="text-[20px] font-bold lg:text-[35px]">20/200</div>
      <div>
        ...
        <Menu as="div" className="relative inline-block text-left">
          ...
          <Menu.Items className="absolute right-0 w-56 mt-2 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
            <div className="px-1 py-1">
              <Menu.Item>
                {({ active }) => (
                  <button className={`${active ? 'bg-darkPurple text-white' : ''} group flex rounded-md items-center w-full px-2 py-2 text-sm text-black`}>
                    <AiFillWarning className="w-5 h-5 mr-2 fill-yellow-400" aria-hidden="true" />
                    Warning
                  </button>
                )}
              </Menu.Item>
              <Menu.Item>
                {({ active }) => (
                  <button className={`${active ? 'bg-darkPurple text-white' : ''} group flex rounded-md items-center w-full px-2 py-2 text-sm text-black`}>
                    <AiOutlinePauseCircle className="w-5 h-5 mr-2" aria-hidden="true" />
                    Pause
                  </button>
                )}
              </Menu.Item>
              <Menu.Item>
                {({ active }) => (
                  <button className={`${active ? 'bg-darkPurple text-white' : ''} group flex rounded-md items-center w-full px-2 py-2 text-sm text-black`}>
                    <AiFillCloseCircle className="w-5 h-5 mr-2 fill-red-500" aria-hidden="true" />
                    Close
                  </button>
                )}
              </Menu.Item>
      ...
      <Modal open={open} handleClose={handleClose} fontSize={fontSize} handleSizeChange={handleSizeChange}/>
    </header>
  );
};
